...
public class StatsUIController : MonoBehaviour
{
    ...
    {
        if (subscribed && StatisticsManager.Instance != null)
        {
            StatisticsManager.Instance.OnStatisticChanged.RemoveListener(OnStatChanged);
            subscribed = false;
        }
    }
    ...
    private void TrySubscribe()
    {
        ...
        StatisticsManager.Instance.OnStatisticChanged.AddListener(OnStatChanged);
        subscribed = true;
        RefreshAll();
    }
    ...
    private void OnStatChanged(string key, object value)
    {
        switch (key)
        {
            ...
            case "caseClicks": UpdateText(totalClicksText, (StatisticsManager.Instance.GetAllStatistics().totalMoneyClicks + StatisticsManager.Instance.GetAllStatistics().totalCaseClicks).ToString("N0")); break;
            case "casesDropped": UpdateText(casesDroppedText, ((int)value).ToString("N0")); break;
            case "casesOpened": UpdateText(casesOpenedText, ((int)value).ToString("N0")); break;
            case "itemsSold": UpdateText(itemsSoldText, ((int)value).ToString("N0")); break;
            case "tradeUps": UpdateText(tradeUpsText, ((int)value).ToString("N0")); break;
            case "upgrades": UpdateText(upgradesText, ((int)value).ToString("N0")); break;
            case "achievements": UpdateText(achievementsText, ((int)value).ToString("N0")); break;
            case "moneyPerSecond": UpdateText(moneyPerSecondText, ((float)value).ToString("F3")); break;
            case "actualMoney": UpdateText(actualMoneyText, "$" + ((double)value).ToString("F2")); UpdateText(moneyRecordText, "$" + StatisticsManager.Instance.GetAllStatistics().moneyRecord.ToString("F2")); break;
            case "totalGainedMoney": UpdateText(totalGainedMoneyText, "$" + ((double)value).ToString("F2")); break;
            case "totalExperienceGained": UpdateText(totalExperienceText, ((int)value).ToString("N0")); break;
    ...
    public void RefreshAll()
    {
        ...
        UpdateText(itemsSoldText, s.totalItemsSold.ToString("N0"));
        UpdateText(tradeUpsText, s.totalTradeUps.ToString("N0"));
        ...
        UpdateText(totalGainedMoneyText, "$" + s.totalGainedMoney.ToString("F2"));
        UpdateText(totalExperienceText, s.totalExperienceGained.ToString("N0"));
        UpdateText(offlineIncomeText, SecondsToHMS(s.offlineIncomeSeconds));
        UpdateText(keysBoughtText, s.totalKeysBought.ToString("N0"));
        UpdateText(missionsCompletedText, s.totalMissionsCompleted.ToString("N0"));
    }
